...
fun SearchExercisesScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.viewEvent.collectLatest { event ->
            when (event) {
                is SearchExercisesViewEvent.OpenExerciseDetails ->
                    navController.navigate(Screens.ExerciseDetailsScreen.route) // pass id
            }
        }
    }
        ...
        LazyColumn(
            ...
            items(viewState.exercises) {
                Card(
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable { viewModel.onExerciseClick(it.id) }
                ) {
